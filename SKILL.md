...
1. Collect input, claim, command, artifact, risk, and verdict events. Set every
   ...
   index. Identify every command with a non-empty `command` or stable `id`, then
   set its status to `pass` with integer exit code `0`, or `fail` with a nonzero
   integer exit code. Anonymous, missing, or inconsistent command evidence is
   rejected with the same event-specific diagnostic.
